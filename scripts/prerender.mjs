...
try {
  ...
  for (const suffix of ["", "en/", "pt/", "es/"]) {
    ...
    await writeFile(
      ...
      `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Raphael Rocha</title><link rel="canonical" href="https://portfolio.raphaelrocha.com${target}"><script>location.replace(${JSON.stringify(target)}+location.search+location.hash)</script><noscript><meta http-equiv="refresh" content="0;url=${target}"></noscript></head><body><a href="${target}">Continue to the portfolio</a></body></html>`,
    );
  }
  const urls = ["en", "pt", "es"]
    .map(
      (locale) =>
        `<url><loc>https://portfolio.raphaelrocha.com/${locale}/</loc></url>`,
    )
    .join("");
  await writeFile(
    "dist/sitemap.xml",
