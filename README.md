...
Clicking the header portrait opens an accessible native dialog with a blurred backdrop. The approved 1254px source and a small WebGL module load only on demand. An 80×80 textured mesh supports up to four local grabs with bounded displacement and damped springs; rendering stops when the surface settles. Original hand cursors, touch capture/cancellation, keyboard region selection, reduced motion, static-image fallback, focus restoration and GPU cleanup are included. The interaction reference is the opening face experiment in the [official Super Mario 64 instruction booklet](https://m1.nintendo.net/docvc/NUS/EUR/NSMP/NSMP_E.pdf); no Nintendo artwork or code is used.
